...
#define RPI_DEF_D7  22
#define RPI_DEF_D6  27
#define RPI_DEF_D5  16
#define RPI_DEF_D4  23
#define RPI_DEF_RS   25
#define RPI_DEF_EN   24
#define RPI_DEF_EN2 21
#define RPI_DEF_BL  20
...
+ * Simple method used for now - based on data found in /proc/iomem and if not present then
+ * based on device found. However, there is a new routine available
...
static volatile unsigned int gpio_top = 0;     /* Top address of GPIO - value not guaranteed */
...
check_board_rev(Driver *drvthis)
{
   ...
   if (((strcmp(hw, "BCM2708") != 0) && (strcmp(hw, "BCM2709") != 0)) || rev == 0) {
       ...
       if ((fp = fopen("/proc/iomem", "r")) != NULL) {
               ...
               if (gpio_base != 0) {
                       report(RPT_INFO, "Found GPIO base via /proc/iomem: 0x%x",gpio_base);
               }
               /* Force to 0 while testing */
               /* gpio_base = 0; */

               fclose(fp);
       }

       /* If not yet worked out the GPIO base address then try another way */
       if (gpio_base == 0) {
